...
class KeyboardInput:
    ...
    def feed_key(self, key: str) -> KeyboardPoll:
        ...
        if key == "u":
            self.numeric_prefix = ""
            return KeyboardPoll(Command.stand_up(), False, notices)
        if key == "k":
            self.numeric_prefix = ""
            return KeyboardPoll(Command.skip_stand_up(), False, notices)
        if key == "j":
            self.numeric_prefix = ""
...
def help_text(controller_name: str) -> str:
    return "\n".join(
        (
            controller_name,
            "Stand up: u",
            "Skip standup (assert already standing): k",
            "Sit down and restore the standup lock: j",
            "Graceful stop: 0",
