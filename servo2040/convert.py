...
def pulse_to_angle(joint: str, pulse: float) -> float:
    """Convert pulse (µs) to angle (degrees) using asymmetric calibration."""
    m_plus, m_minus = CALIBRATION[joint]
    if pulse >= CENTER_PULSE:
        return (pulse - CENTER_PULSE) / m_plus
    else:
        return (pulse - CENTER_PULSE) / m_minus
...
if __name__ == "__main__":
    ...
    joint = "j12"
    pulse = 1720
    angle = pulse_to_angle(joint, pulse)
    print(f"{joint}: {pulse} → {angle:.0f} degrees")
    joint = "j13"
    pulse = 1880
    angle = pulse_to_angle(joint, pulse)
    print(f"{joint}: {pulse} → {angle:.0f} degrees")
